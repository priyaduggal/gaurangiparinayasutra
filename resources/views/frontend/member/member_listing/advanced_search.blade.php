            ...
            <div class="pb-4">
                <form action="{{ route('member.listing') }}" method="get">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="form-group mb-3">
                                ...
                                <select name="caste_id[]" id="caste_id" class="form-control aiz-selectpicker"  multiple data-live-search="true" >
                                      <?php $educations = \App\Models\Country::castes(); ?>
                                        @foreach ($educations as $country)
                                        <option value="{{$country->id}}"  <?php if(count($caste_id)>0 && in_array($country->id,$caste_id)) echo 'selected';?>>{{$country->name}}</option>
                                        @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="form-group mb-3">
                                <label class="form-label" for="name">{{ translate('Education') }}</label>
                                <select name="education_id[]" id="education_id" class="form-control aiz-selectpicker"  multiple data-live-search="true" >
                                        <?php $educations = \App\Models\Country::educations(); ?>
                                        @foreach ($educations as $country)
                                        <option value="{{$country->id}}" <?php if(count($education_id)>0 && in_array($country->id,$education_id)) echo 'selected';?>>{{$country->name}}</option>
                                        @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="form-group mb-3">
                                <label class="form-label" for="name">{{ translate('Occupation') }}</label>
                                <select name="occupation_id[]" id="occupation_id" class="form-control aiz-selectpicker"  multiple data-live-search="true" >
                                     <?php $countries = \App\Models\Country::occupation(); ?>
                        @foreach ($countries as $country)
                            <option value="{{$country->id}}" <?php if(count($occupation_id)>0 && in_array($country->id,$occupation_id)) echo 'selected';?>>{{$country->name}}</option>
                        @endforeach
                                </select>
                            </div>
                        </div>
