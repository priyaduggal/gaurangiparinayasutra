...
class HomeController extends Controller
{
    ...
    public function member_listing(Request $request)
    {
        ...
        $caste_id       = ($request->caste_id != null) ? $request->caste_id : array();
        $occupation_id   = ($request->occupation_id != null) ? $request->occupation_id : array();
        $education_id   = ($request->education_id != null) ? $request->education_id : array();
        ...
        $onion    = ($request->onion != null) ? $request->onion : '';
        $garlic    = ($request->garlic != null) ? $request->garlic : '';
       ...
       // print_r($users);die;
        ...
        $users = $users->WhereNotIn("id", function ($query){
            ...
            $query->select('ignored_by')
                ...
                if($onion!=null)
                        {
                             $users = $users->where('onion', $onion);
                        }
                if($garlic!=null)
                        {
                             $users = $users->where('garlic', $garlic);
                        }
        ...
        elseif (!empty($education_id)) {
             $user_ids = PhysicalAttribute::whereIn('weight', $education_id)->pluck('user_id')->toArray();
            $users = $users->WhereIn('id', $user_ids);
        }
       
        elseif (!empty($occupation_id)) {
            
             $user_ids = PhysicalAttribute::whereIn('eye_color', $occupation_id)->pluck('user_id')->toArray();
            $users = $users->WhereIn('id', $user_ids);
        }
        
        elseif (!empty($caste_id)) {
           
            $user_ids = SpiritualBackground::whereIn('caste_id', $request->caste_id)->pluck('user_id')->toArray();
            $users = $users->WhereIn('id', $user_ids);
        }
        ...
        return view('frontend.member.member_listing.index', compact('occupation_id','education_id','users','onion','garlic','age_from','age_to','member_code','matital_status','religion_id','caste_id','sub_caste_id','mother_tongue','profession','country_id','state_id','city_id','min_height','max_height','member_type'));
    }
